...
RegisterNetEvent("doj:client:startingBets", function(args)
    ...
    if waitingForBetState then
        if args == 1 then 
            ...
            if currentBetAmount < maxbet and currentBetAmount > minbet then
                QBCore.Functions.Notify('Bet raised', 'success')
                currentBetAmount = currentBetAmount + 100
            else
                QBCore.Functions.Notify('Cant bet higher', 'error')
            end
        elseif args == 2 then 
            -- -- print("Bet lowered")
            TriggerEvent("doj:client:openBetMenu")
            ...
            if currentBetAmount < maxbet and currentBetAmount > minbet then
                ...
                QBCore.Functions.Notify('Cant bet lower', 'error')
            end
        elseif args == 3 then 
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    minbet = cfg.blackjackTables[tableId].limits.min
    maxbet = cfg.blackjackTables[tableId].limits.max
