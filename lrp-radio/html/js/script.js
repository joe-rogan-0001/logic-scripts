$(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) { // Escape key
            $.post('https://lrp-radio/escape', JSON.stringify({}));
            QBRadio.SlideDown()
        } else if (data.which == 13) { // Enter key
            $.post('https://lrp-radio/joinRadio', JSON.stringify({
                channel: $("#channel").val()
            }));
        }
    };
...
$(document).on('click', '#disconnect', function(e){
    ...
    $.post('https://lrp-radio/leaveRadio');
});

$(document).on('click', '#volumeUp', function(e){
    e.preventDefault();

    $.post('https://lrp-radio/volumeUp', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#volumeDown', function(e){
    ...
    $.post('https://lrp-radio/volumeDown', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#decreaseradiochannel', function(e){
    ...
    $.post('https://lrp-radio/decreaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#increaseradiochannel', function(e){
    ...
    $.post('https://lrp-radio/increaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#poweredOff', function(e){
    ...
    $.post('https://lrp-radio/poweredOff', JSON.stringify({
        channel: $("#channel").val()
    }));
});
