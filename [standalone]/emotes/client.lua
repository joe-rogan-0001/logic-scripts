...
AddEventHandler("expressions:clear",
                function() ClearFacialIdleAnimOverride(PlayerPedId()) 
end)

RegisterCommand('e', function(source, args, raw) 
    local dead = false
    if not IsPedSittingInAnyVehicle(PlayerPedId()) and not dead then
    TriggerEvent('animation:PlayAnimation', args[1]) 
    else 
        TriggerEvent("DoLongHudText", "Can't Emote At This Time")
    end
end)
