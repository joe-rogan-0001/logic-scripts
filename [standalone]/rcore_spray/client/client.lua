...
function RenderSortedSprays(coords, s)
    local closeSprays = {}
    local closeSprayDistances = {}
    local scaleformHandleIdx = SCALEFORM_ID_MIN

    for _, spray in pairs(s) do
        if (spray.color or spray.image) then
            local dist = #(coords - spray.realLocation)
            if dist < 25.0 then
                table.insert(closeSprays, {dist, spray})
                table.insert(closeSprayDistances, dist)
            end
        end
    end

    table.sort(closeSprayDistances)
    local scaleformBudget = SCALEFORM_ID_MAX-SCALEFORM_ID_MIN

    for i = 1, math.min(#closeSprayDistances, scaleformBudget) do
        for _, s in pairs(closeSprays) do
            if math.abs(s[1] - closeSprayDistances[i]) < 0.001 then
                --print(PLAYER_NAME_HEAP[scaleformHandleIdx], json.encode(s[2]))
                onScreen = World3dToScreen2d(s[2].location.x, s[2].location.y, s[2].location.z)
                
                if onScreen then
                    DrawSpray(PLAYER_NAME_HEAP[scaleformHandleIdx], s[2])
...
function checkifonscreen(coords, s)
    local closeSprays = {}
    local closeSprayDistances = {}
    local scaleformHandleIdx = SCALEFORM_ID_MIN
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if onScreen then
            ...
            RenderSortedSprays(coords, SPRAYS)
