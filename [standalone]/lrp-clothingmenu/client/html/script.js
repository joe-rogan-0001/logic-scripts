...
$(function () {
    ...
    function CloseMenu(save, paymentType) {
        $.post('https://lrp-clothingmenu/escape', JSON.stringify({
          save: save,
          paymentType,
    ...
    $(document).on('contextmenu', function() {
        $.post('https://lrp-clothingmenu/togglecursor', JSON.stringify({}));
    })
    ...
    $('#skin_string').find('input').keypress(function (e) {
        if (e.which == 13) {
            $.post('https://lrp-clothingmenu/customskin', JSON.stringify($(this).val()));
        }
    })
    ...
    function inputChange(ele,inputType) {
        ...
        if (currentMenu.is($('#clothing_shop')) || $(ele).parents('.panel').hasClass('hair')) {
            ...
            if ($(ele).parents('.panel').attr('id') == "skins") {
                $.post('https://lrp-clothingmenu/setped', JSON.stringify({
                    "name": $(ele).attr('id'),
                    "value": $(ele).val()
                }))
            }
                ...
                $.post('https://lrp-clothingmenu/updateclothes', JSON.stringify({
                    "name": nameId,
            ...
            $.post('https://lrp-clothingmenu/settats', JSON.stringify({tats}))
        }
    }
    ...
    $('.tog_head').on('click', function() {
        toggleCam($(this));
        $.post('https://lrp-clothingmenu/switchcam', JSON.stringify({name: 'head'}))
    })
    $('.tog_torso').on('click', function() {
        toggleCam($(this));
        $.post('https://lrp-clothingmenu/switchcam', JSON.stringify({name: 'torso'}))
    })
    $('.tog_leg').on('click', function() {
        toggleCam($(this));
        $.post('https://lrp-clothingmenu/switchcam', JSON.stringify({name: 'leg'}))
    })
    $('.tog_cam').on('click', function() {
        toggleCam($(this));
        $.post('https://lrp-clothingmenu/switchcam', JSON.stringify({name: 'cam'}))
    })

    $('.backup_save').on('click', function() {
        $.post('https://lrp-clothingmenu/getPlayerFace')
    })
    ...
    $('.backup_load').on('click', function() {
        $.post('https://lrp-clothingmenu/setPlayerFace',$("#backupJsonInput").val())
    })

    $('.tog_hat').on('click', function() {
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "hats"}))
    })
    $('.tog_glasses').on('click', function() {
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "glasses"}))
    })
    $('.tog_tops').on('click', function() {
        // dont look at this :)
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "jackets"}))
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "undershirts"}))
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "torsos"}))
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "vest"}))
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "bags"}))
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "neck"}))
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "decals"}))
    })
    $('.tog_legs').on('click', function() {
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "legs"}))
    })
    $('.tog_mask').on('click', function() {
        $.post('https://lrp-clothingmenu/toggleclothes', JSON.stringify({name: "masks"}))
    })

    $('#reset').on('click', function() {
        $.post('https://lrp-clothingmenu/resetped', JSON.stringify({}))
    })
    ...
    window.addEventListener("keydown", throttle(function (ev) {
        ...
        if (_key) {
            $.post('https://lrp-clothingmenu/rotate', JSON.stringify({key: _key}))
        }
    }, 50))
    ...
    function SaveHeadBlend() {
        ...
        $.post('https://lrp-clothingmenu/saveheadblend', JSON.stringify(headBlend))
    }

    function SaveFaceShape(ele) {
        $.post('https://lrp-clothingmenu/savefacefeatures', JSON.stringify({name: ele.attr('data-value'), scale: ele.val()}))
    }
    ...
    function SaveEyeColor() {
        $.post('https://lrp-clothingmenu/saveeyecolor', JSON.stringify({
    ...
    function SaveHeadOverlay(ele) {
        ...
        $.post('https://lrp-clothingmenu/saveheadoverlay', JSON.stringify({
            name: id,
            value: inputs.eq(0).val(),
            opacity: opacity
        }))
    ...
    function AddPalettes() {
        ...
        $('.color_palette').on('click', function() {
            ...
            if ($(this).parents('.panel').hasClass('hair')) {
                $.post('https://lrp-clothingmenu/savehaircolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value')
                }));
            }
            else {
                $.post('https://lrp-clothingmenu/saveheadoverlaycolor', JSON.stringify({
                    firstColour: palettes.eq(0).find('.active').attr('value'),
                    secondColour: palettes.eq(1).find('.active').attr('value'),
