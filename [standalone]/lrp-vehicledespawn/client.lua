...
AddEventHandler('vehicledespawn:save', function(vehicle)
	local model = GetEntityModel(vehicle)
	local x,y,z = table.unpack(GetEntityCoords(vehicle))
	local heading = GetEntityHeading(vehicle)
	local mods = QBCore.Functions.GetVehicleProperties(vehicle)
	TriggerServerEvent('vehicledespawn:save', vehicle, model, x, y, z, heading, mods)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsPedInAnyVehicle(ped) then
			...
			TriggerEvent('vehicledespawn:save', vehicle)
			SetVehicleHasBeenOwnedByPlayer(vehicle, true)
...
RegisterNetEvent('vehicledespawn:checkVehs')
AddEventHandler('vehicledespawn:checkVehs', function(table)
	dprint("Checking for vehicles to restore..")
	local results = {
		['restored'] = 0,
		['total'] = 0
	}
	for i=1,#table,1 do
		if GetEntityModel(table[i].id) ~= table[i].model then
			local newId = spawnVehicle(table[i].model, table[i].x, table[i].y, table[i].z, table[i].heading)
			QBCore.Functions.SetVehicleProperties(newId, table[i].mods)
			TriggerServerEvent('vehicledespawn:updateId', table[i].id, newId)
...
Citizen.CreateThread(function()
	while true do
		...
		TriggerServerEvent('vehicledespawn:retrieveTable')
		...
		while true do
			if IsPedInAnyVehicle(ped) then
				if not inVehicle then
					TriggerEvent('vehicledespawn:save', GetVehiclePedIsUsing(ped))
				end
				inVehicle = true
			else
				inVehicle = false
