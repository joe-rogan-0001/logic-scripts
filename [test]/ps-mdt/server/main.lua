...
local MugShotWebhook = 'https://discord.com/api/webhooks/1157763826222125076/9LYYXIiVcE69DYgSG4JeUuGmoBZwEn3XPNK4Qpj-OorbNPKnTsOZ80q7XrzTKoj1en2D'

-- Clock-in notifications for duty. Add a Discord webhook.
-- Command /mdtleaderboard, will display top players per clock-in hours.
local ClockinWebhook = 'https://discord.com/api/webhooks/1157764116941901824/ixbmCGplDfHuQq5mceZa-wrBMRvlWH6lBPuFQBU6YT-2EPNYWKWcRcGeKWzeFMFj8L04'
--------------------------------
...
if Config.UseWolfknightRadar == true then
	...
	AddEventHandler("wk:onPlateScanned", function(cam, plate, index)
		...
		local vehInfo = GetVehicleInformation(plate)
		...
		if vehInfo then
			if vehInfo.stolen then
				TriggerClientEvent('QBCore:Notify', src, 'STOLEN VEHICLE  | Registered Owner: '..vehicleOwner..' | Plate: '..plate, 'error', Config.WolfknightNotifyTime)
...
RegisterNetEvent("ps-mdt:server:OnPlayerLoaded", function()
	local src = source
    local PlayerData = GetPlayerData(src)
    local time = os.date("%Y-%m-%d %H:%M:%S")
    local firstName = PlayerData.charinfo.firstname:sub(1,1):upper()..PlayerData.charinfo.firstname:sub(2)
