...
--[[Citizen.CreateThread(function()
    while true do
        ...
        if IsControlPressed(0, 322) then  -- 322 corresponds to the 'Escape' key
            CloseDialog()
        end

        -- Check if the 'Backspace' key is pressed
        if IsControlPressed(0, 177) then  -- 177 corresponds to the 'Backspace' key
            CloseDialog()
        end
    end
end)]]--
...
DeletePedByID = function(id) 
    if Peds[id].ped then
        exports['lrp-target']:RemoveTargetEntity(Peds[id].ped)
