...
local function showSellMenu(ped, item, amt, price)
		...
		lib.registerContext({
			...
			options = {
				...
				},
				{
					...
					icon = 'circle-check',
					onSelect = function()
						local success = exports['SN-Hacking']:SkillBar({3000, 4000}, 8, 2)
						if success then TriggerEvent('cad-drugsales:salesinitiate', { type = 'buy', item = item, price = price, amt = amt, tped = ped })
						else TriggerEvent('cad-drugsales:salesinitiate', { type = 'close', tped = ped })
					end
				end
				},
				{
	...
	SetTimeout(Config.SellTimeout*1000, function()
		if InitiateSellProgress then
			...
			exports['lrp-notify']:Alert("ERROR", "You took too long...", 5000, 'error')
...
local function initiateSell(ped)
	...
	if not hasSold then
		SetPedAsNoLongerNeeded(ped)
		exports['lrp-notify']:Alert("ERROR", "Nothing to sell!", 5000, 'error')
...
local function InteractPed(ped)
	...
	if Playerjob.name == 'police' then
		exports['lrp-notify']:Alert("ERROR", "Fuck off you pig!", 5000, 'error')
		SetPedAsNoLongerNeeded(ped)
	...
	if percent < Config.ChanceSell then
		...
		exports['lrp-notify']:Alert("ERROR", "Buyer is calling the police!", 5000, 'error')
...
local function initiateSales(entity)
	...
	if count < Config.MinimumCops then
		exports['lrp-notify']:Alert("ERROR", "Buyer is not interested right now!", 5000, 'error')
		if Config.Debug then print('Not Enough Cops') end
	else
		local netId = NetworkGetNetworkIdFromEntity(entity)
		local isSoldtoPed = hasSoldPed(netId)
		if isSoldtoPed then exports['lrp-notify']:Alert("ERROR", "You have already spoken to this person!", 5000, 'error') return false end
		addSoldPed(netId)
		InteractPed(entity)
...
RegisterNetEvent('cad-drugsales:notify', function(msg)
	exports['lrp-notify']:Alert("SUCCESS", (msg), 5000, 'success')
...
RegisterNetEvent('cad-drugsales:salesinitiate', function(cad)
	if cad.type == 'close' then
		InitiateSellProgress = false
		if Config.OxMenu then lib.hideMenu() else lib.hideContext() end
		exports['lrp-notify']:Alert("ERROR", "You rejected the offer!", 5000, 'error')
