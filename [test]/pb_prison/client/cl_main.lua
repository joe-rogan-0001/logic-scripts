...
RegisterNetEvent('qb-prison:client:SendPlayerToPrison', function(takeMugshot, name, time, citizenid, date)
    local ped = PlayerPedId()
    if not DoesEntityExist(ped) then return end
...
RegisterNetEvent('qb-jail:client:PhoneCall', function()
    local dialog = exports['lrp-menu']:ShowInput({
    ...
    elseif Config.Phone == 'ICY' then
        local calldata = {
            number = dialog.number,
            name = dialog.number
        }
        exports['pb_phone']:CallContact(calldata , true) -- You need to create this export in qb-phone, see readme
...
RegisterNetEvent('qb-jail:client:PrisonRevive', function()
    if PlayerData.metadata.injail ~= 0 or PlayerData.job.type == 'leo' then
        if exports['pb_police']:IsHandcuffed() then
