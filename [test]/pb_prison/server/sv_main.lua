...
local jailPlayer = function(src, takeItems, name, time, citizenid, date) 
    ...
    TriggerClientEvent('qb-prison:client:SendPlayerToPrison', src, false, name, time, citizenid, date)
    print("working here")
    if takeItems and (not Player.PlayerData.metadata['jailitems'] or table.type(Player.PlayerData.metadata['jailitems']) == 'empty') then
        ...
        if Player.Functions.RemoveMoney('cash', cash, 'prison-belongings') then
            print("put moeny in players bank")
...
RegisterNetEvent('pb_prison:server:JailPlayer', function(playerId, takeItems, name, time, citizenid, currentDate)
    jailPlayer(playerId, takeItems, name, time, citizenid, currentDate)
end)
...
local unJailPlayer = function(src)
    ...
    if Config.Inventory == 'exports' then
        print("IM GAY AS FUCK FOR USING pb-inventory")
    ...
    TriggerClientEvent('QBCore:Notify', src, 'You have been released from prison', 'primary', 4000)
...
AddEventHandler('onResourceStart', function(resource)
    ...
    for k, v in pairs(QBCore.Functions.GetQBPlayers()) do
        ...
        local time = v.PlayerData.metadata.injail
        if time ~= 0 then
            TriggerClientEvent('QBCore:Notify', v.PlayerData.source, 'Jail Script Restarting', 'primary', 4000)
            jailPlayer(v.PlayerData.source, false, name, time, citizenid, os.date('%d-'..'%m-'..'%y'))
        end
    end
end)
...
RegisterNetEvent('QBCore:Server:OnPlayerLoaded', function()
    ...
    if not Player then return end
    if time ~= 0 then
        jailPlayer(src, false, name, time, citizenid, os.date('%d-'..'%m-'..'%y'))
    end
end)
...
QBCore.Commands.Add('confiscate', 'Confiscate Player Items', {{name = 'id', help = 'Player ID'}}, true, function(source, args)
    ...
    if Player and OtherPlayer and Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then -- if Player and OtherPlayer and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty then
        ...
        if #(GetEntityCoords(ped) - GetEntityCoords(targetPed)) < 2.5 then
            if OtherPlayer.PlayerData.metadata["ishandcuffed"] or OtherPlayer.PlayerData.metadata["isdead"] or OtherPlayer.PlayerData.metadata["isunconscious"] then
    ...
    if Player and OtherPlayer and Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then -- if Player and OtherPlayer and Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty then
        ...
        if #(GetEntityCoords(ped) - GetEntityCoords(targetPed)) < 2.5 then
            if OtherPlayer.PlayerData.metadata['jailitems'] and table.type(OtherPlayer.PlayerData.metadata['jailitems']) ~= 'empty' then
                if Config.Inventory == 'exports' then
                    exports['pb_inventory']:ClearInventory(playerId, nil)
                    Wait(1000)
                    for _, v in pairs(OtherPlayer.PlayerData.metadata['jailitems']) do
                        exports['pb_inventory']:AddItem(playerId, v.name, v.amount, false, v.info)
                    end
