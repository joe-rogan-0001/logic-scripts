...
QBCore.Functions.CreateUseableItem("prisonslushie", function(source, item)
    ...
    if Config.Inventory == 'exports' then
        if exports['pb_inventory']:RemoveItem(src, 'prisonslushie', 1, false) then
            ...
            if Config.SlushieSpoonChance >= math.random(100) and isPlayerInJail(src) then
                exports['pb_inventory']:AddItem(src, 'prisonspoon', 1, false)
...
RegisterNetEvent('qb-jail:server:CraftItem', function(index)
    ...
    if hasItems then
        for _, v in pairs(Config.CraftingCost[index].items) do
            if Config.Inventory == 'exports' and exports['pb_inventory']:RemoveItem(src, v.item, v.amount, false) then
        ...
        if Config.Inventory == 'exports' and exports['pb_inventory']:AddItem(src, Config.CraftingCost[index].item, Config.CraftingCost[index].amount, false) then
...
RegisterNetEvent('qb-jail:server:PurchaseItem', function(index)
    ...
    if QBCore.Functions.HasItem(src, Config.PrisonerPedShop[index].cost.item, Config.PrisonerPedShop[index].cost.amount) then
        if Config.Inventory == 'exports' and exports['pb_inventory']:RemoveItem(src, Config.PrisonerPedShop[index].cost.item, Config.PrisonerPedShop[index].cost.amount, false) then
        ...
        if Config.Inventory == 'exports' and exports['pb_inventory']:AddItem(src, Config.PrisonerPedShop[index].item, 1, false) then
