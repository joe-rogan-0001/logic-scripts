...
RegisterNUICallback('getCamsData', function(_, cb)
	Wait(1000)
	local Screenshots = GetResourceKvpString('LRPScreenshots')
	...
	print('^5[cameramode] ^7Successfully loaded Camera Mode Settings!')
    cb(data)
end)
...
RegisterNUICallback('SavePicture', function(data)
	...
	SetResourceKvp('LRPScreenshots', json.encode(SavedPictures))
end)
...
RegisterNetEvent('cameramode:client:OpenCameraMode', function()
    OpenCameraMode()
end)
