...
$(document).ready(() => {
    ...
    $(".gallery-add-btn").click(function() {
        if ($(".manage-profile-citizenid-input").val() && civperson == false) {
    ...
    $(".manage-incidents-title-holder").on("click", ".manage-incidents-create", function() {
        let tempalte = "LSPD Report | " + $(".date").html() + " " + $(".time").html() + "\n\nReporting Officer:\n\nWitnesses:\n\nLocation:\n\nDebrief:"
        $("#manage-incidents-title-input").val("Crime | " + $(".date").html());
    ...
    $(".icidents-person-search-holder").on("click", ".icidents-person-search-item", function() {
        ...
        $(".incidents-ghost-holder").prepend(
            ...
            <div class="associated-incidents-user-container" data-id="${$(this).data("cid")}">
                ...
                <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Final Fine</div>
                ...
                <div class="manage-incidents-title-tag" data-id="${$(this).data("cid")}">Final Sentence</div>
    ...
    $(".contextmenu").on("click", ".set-waypoint", function () { 
        let info = $(this).data("info")
        console.log(info)
        $.post('https://lrp-mdt/setWaypointU', JSON.stringify({
            cid: info,
        }));
    })
    ...
    function JobColors(sentJob) {
        if (sentJob) {
            if (sentJob == 'police') {
                ...
                $(".badge-logo").attr('src', 'https://static.wikia.nocookie.net/nopixel/images/4/43/LSPD3.png/');
                ...
                $(".quote-span").html("'Who thought adding a quote was a good idea' - Miles Austin");
                ...
                $(".gallery-upload-input").hide()
                $(".header-right").hide()
                //document.getElementsByClassName(".manage-profile-info").readOnly = true;
                //$(".weapons-add-btn").hide()
                $(".gallery-add-btn fas fa-plus").hide()
    ...
    window.addEventListener('message', function(event) {
        ...
        if (e.type == "show") {
            ...
            if (table["profilepic"] == null) {
                table["profilepic"] = 'https://media.discordapp.net/attachments/832371566859124821/872590513646239804/Screenshot_1522.png'
            }
            ...
            var metadata = jQuery.parseJSON(table.metadata)
            let driver = ""
            if (metadata.licences.driver == true) {
                driver = "green-tag";
            } else {
                driver = "red-tag";
            }
            let permit = ""
            if (metadata.licences.permit == true) {
                permit = "green-tag";
            } else {
                permit = "red-tag";
            }
            let weapon = ""
            if (metadata.licences.weapon == true) {
                weapon = "green-tag";
            } else {
                weapon = "red-tag";
            }
            let hunting = ""
            if (metadata.licences.hunting == true) {
                hunting = "green-tag";
            } else {
                hunting = "red-tag";
            }
            let fishing = ""
            if (metadata.licences.fishing == true) {
                fishing = "green-tag";
            } else {
                fishing = "red-tag";
            }
            let pilot = ""
            if (metadata.licences.pilot == true) {
                pilot = "green-tag";
            } else {
                pilot = "red-tag";
            }
            let cdl = ""
            if (metadata.licences.cdl == true) {
                cdl = "green-tag";
            } else {
                cdl = "red-tag";
            }
            let business = ""
            if (metadata.licences.business == true) {
                business = "green-tag";
            } else {
                business = "red-tag";
            }
            ...
            $('.licenses-holder').prepend(
                `
            <div class="license-tag ${permit} permit">Permit</div>
            <div class="license-tag ${driver} driver">Drivers</div>
            <div class="license-tag ${cdl} cdl">CDL</div>
            ...
            if (table["vehicles"]) {
                $.each(table["vehicles"], function(index, value) {
                    $('.vehs-holder').prepend(`<div class="veh-tag" data-plate="${value.plate}">${value.plate} - ${value.vehicle} </div>`);
                })
            }
            ...
            const alreadyput = new Array();

            const counts = {};
            table["convictions"].forEach(function (x) { counts[x] = (counts[x] || 0) + 1; });

            $(".convictions-holder").empty();
            if (table["convictions"]) {
                $.each(table["convictions"], function(index, value) {    
                    if (jQuery.inArray(value, alreadyput) == -1) {
                        $('.convictions-holder').prepend(`<div class="white-tag">${value + ' ' + '(' + (counts[value]) + ')'} </div>`);
                        alreadyput.push(value);
                    }
                })
            }
            ...
            $.each(table, function(index, value) {
                ...
                let warrant = ""
                let convictions = ""
                var metadata = jQuery.parseJSON(value.metadata)
                //console.table(JSON.stringify(metadata.licences.driver))
                if (metadata.licences.permit == true) {
                    permit = "green-tag";
                } else {
                    permit = "red-tag";
                ...
                <div class="profile-tag ${permit}">Permit</div>
                <div class="profile-tag ${driver}">Drivers</div>
                <div class="profile-tag ${cdl}">CDL</div>
            ...
            if (value) {
                ...
                //var origin = jQuery.parseJSON(value)

                //console.table(JSON.stringify(value.origin.x))
                let DispatchItem = `<div class="active-calls-item" data-id="${value.callId}" data-x="${value.origin.x}" data-y="${value.origin.y}" data-canrespond="false"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#${value.callId}</div><div class="call-code priority-${prio}">${value.dispatchCode}</div><div class="call-title">${value.dispatchMessage}</div></div><div class="call-item-bottom">`
                if (value.dispatchCode == "911" || value.dispatchCode == "311" || value.dispatchCode == "10-13A" || value.dispatchCode == "211") {
                    DispatchItem = `<div class="active-calls-item" data-id="${value.callId}"  data-x="${value.origin.x}" data-y="${value.origin.y}" data-canrespond="true"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#${value.caller}</div><div class="call-code priority-${prio}">${value.dispatchCode}</div><div class="call-title">${value.dispatchMessage}</div></div><div class="call-item-bottom">`
                }
            ...
            $.each(table, function(index, value) {
                if (value) {
                    const prio = value['priority']
                    //console.table(JSON.stringify(value))
                    let DispatchItem = `<div class="active-calls-item" data-id="${value.callId}" data-x="${value.x}" data-y="${value.y}" data-canrespond="false"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#${value.callId}</div><div class="call-code priority-${prio}">${value.dispatchCode}</div><div class="call-title">${value.dispatchMessage}</div></div><div class="call-item-bottom">`
                    if (value.dispatchCode == "911" || value.dispatchCode == "311" || value.dispatchCode == "10-13A" || value.dispatchCode == "211") {
                        DispatchItem = `<div class="active-calls-item" data-id="${value.callId}"  data-x="${value.coords.x}" data-y="${value.coords.y}" data-canrespond="true"><div class="active-call-inner-container"><div class="call-item-top"><div class="call-number">#${value.caller}</div><div class="call-code priority-${prio}">${value.dispatchCode}</div><div class="call-title">${value.dispatchMessage}</div><div class="call-radio">${value.units.length}</div></div><div class="call-item-bottom">`
                    }
                ...
                if (value.associated == true) {
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Final Fine</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Final Sentence</div>
                    ...
                    $(".incidents-ghost-holder").prepend(
                        `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Final Fine</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Final Sentence</div>
            ...
            $.each(table, function(index, value) {
                ...
                $(".icidents-person-search-holder").prepend(
                    `
                    <div class="icidents-person-search-item" data-info="${name} (#${value.citizenid})" data-cid="${value.citizenid}" data-name="${name}">
                        ...
                        <div class="icidents-person-search-item-right">
                            ...
                            <div class="icidents-person-search-item-right-cid-input"><span class="fas fa-id-card"></span> ${value.citizenid}</div>
            ...
            //console.table(JSON.stringify(table))
            ...
            $.each(table, function(index, value) {
                ...
                //console.log(value.vehicle_state)
                ...
                <div class="dmv-item" data-id="${value.id}" data-dbid="${value.dbid}" data-plate="${value.plate}">
                    ...
                    <div style="display: flex; flex-direction: column;">
                        ...
                        <div class="dmv-bottom-info">
                            <div class="dmv-id">ID: ${value.cid} · Owner: ${value.owner}</div>
                        </div>
                    </div>
                </div>
                `)
