...
function EnableGUI(enable)
    ...
    SendNUIMessage({ type = "show", enable = enable, job = job })
...
function RefreshGUI()
    ...
    SendNUIMessage({ type = "show", enable = false, job = job })
    isOpen = false
end
...
RegisterCommand("refreshmdt", function(source, args, rawCommand)
	RefreshGUI()
end, false)

RegisterNetEvent("mdt:civ:open")
AddEventHandler("mdt:civ:open", function()
    EnableGUI(true)
end)

RegisterNetEvent("mdtopen:radialc")
AddEventHandler("mdtopen:radialc", function()
    TriggerServerEvent('mdtopen:radial')
end)
...
AddEventHandler('np-mdt:getProfileData', function(sentData)
    ...
    for i = 1, #vehicles do
        ...
        if tempModel and tempModel ~= "Unknown" then
            local DisplayNameModel = QBCore.Shared.Vehicles[tempModel['vehicle']]["name"] 
            local LabelText = QBCore.Shared.Vehicles[tempModel['vehicle']]["name"] 
...
AddEventHandler('np-mdt:searchVehicles', function(sentData)
    for i=1, #sentData do
        local mods = json.decode(sentData[i].mods)
        sentData[i]["cid"] = sentData[i].id
        sentData[i]["owner"] = sentData[i].owner
        sentData[i]['plate'] = string.upper(sentData[i]['plate'])
        sentData[i]['color'] = ColorInformation[mods.color1]
        sentData[i]['colorName'] = ColorNames[mods.color1]
        sentData[i]["code5"] = sentData[i].code5
        sentData[i]["stolen"] = sentData[i].stolen
        sentData[i]['model'] = sentData[i]['vehicle']
...
AddEventHandler('np-mdt:getVehicleData', function(sentData)
    if sentData and sentData[1] then
        ...
        vehicle['color'] = ColorInformation[mods.color1]
        vehicle['colorName'] = ColorNames[mods.color1]
        vehicle['model'] = QBCore.Shared.Vehicles[vehicle['vehicle']]["name"]
        vehicle['class'] = classlist[GetVehicleClassFromName(vehicle['vehicle'])]
        --print(classlist[GetVehicleClassFromName(vehicle['vehicle'])])
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    TriggerServerEvent('np-mdt:saveVehicleInfo', dbid, stolen, plate, imageurl, notes)
    cb(true)
end)
...
RegisterNUICallback("getPenalCode", function()
    TriggerEvent('np-mdt:getPenalCode')
    --cb(true)
end)

RegisterNetEvent('np-mdt:getPenalCode')
AddEventHandler('np-mdt:getPenalCode', function()
    --print(dump(Config["Offences"]))
    SendNUIMessage({ type = "getPenalCode", titles = Config["Titles"], penalcode = Config["Offences"] })
end)
...
RegisterNUICallback("removeCall", function(callid, cb)
...
RegisterNUICallback('setWaypoint', function(data, cb)
    ...
    exports['lrp-notify']:Alert("Dispatch", "GPS Marked!", 5000, 'info')
...
AddEventHandler('dispatch:clNotify', function(data, calls)
