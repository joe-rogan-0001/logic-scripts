...
function EndJob()
	if Targets["Package"] then exports["lrp-target"]:RemoveTargetEntity(randPackage) end
...
function ClearProps()
	...
	if Targets["DropOff"] then exports["lrp-target"]:RemoveTargetEntity(TrollyProp) end
...
function PickRandomPackage(Trolly)
	...
	if Targets["Package"] then exports["lrp-target"]:RemoveTargetEntity(randPackage, "Search") end
...
RegisterNetEvent("jim-recycle:PickupPackage:Start", function(data) local Ped = PlayerPedId()
	...
	if progressBar({label = Loc[Config.Lan].progressbar["search"], time = 5000, cancel = true }) then
...
RegisterNetEvent("jim-recycle:PickupPackage:Hold", function(data) local Ped = PlayerPedId()
	...
	exports["lrp-target"]:RemoveTargetEntity(randPackage, "Search")
...
RegisterNetEvent("jim-recycle:PickupPackage:Finish", function(data) local Ped = PlayerPedId()
	...
	if Targets["DropOff"] then exports["lrp-target"]:RemoveTargetEntity(TrollyProp, Loc[Config.Lan].target["drop_off"]) Targets["DropOff"] = nil end
	destroyProp(TrollyProp) SetEntityDrawOutline(TrollyProp, false) TrollyProp = nil
	--Remove target and the whole prop, seen as how no ones lrp-target works and its my fault 😊
...
RegisterNetEvent('jim-recycle:Selling:Menu', function(data)
	...
	if Config.Menu == "qb" then
		sellMenu[#sellMenu+1] = {header = Loc[Config.Lan].menu["sell_mats"], txt = Loc[Config.Lan].menu["sell_mats_txt"], isMenuHeader = true }
		sellMenu[#sellMenu+1] = {header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-recycle:CloseMenu" } }
	end
...
RegisterNetEvent('jim-recycle:Trade:Menu', function(data)
	...
	local tradeMenu = {}
	if Config.Menu == "qb" then
		tradeMenu[#tradeMenu+1] = { header = Loc[Config.Lan].menu["mats_trade"], isMenuHeader = true }
		tradeMenu[#tradeMenu+1] = {header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-recycle:CloseMenu" } }
	end
	local tradeTable = {}
	...
	elseif Config.Menu == "qb" then	exports['lrp-menu']:openMenu(tradeMenu) end
	lookEnt(data.Ped)
end)
...
RegisterNetEvent('jim-recycle:Bottle:Menu', function(data)
	...
	local tradeMenu = {}
	if Config.Menu == "qb" then
		tradeMenu[#tradeMenu+1] = { header = Loc[Config.Lan].menu["sell_mats"], txt = Loc[Config.Lan].menu["sell_mats_txt"], isMenuHeader = true }
		tradeMenu[#tradeMenu+1] = { header = "", txt = Loc[Config.Lan].menu["close"], params = { event = "jim-recycle:CloseMenu" } }
	end
