...
CreateThread(function()
    for k, v in pairs(Config.ElevatorButton) do
        exports['lrp-target']:AddBoxZone(v.name, v.location, v.width, v.length, {
            name = v.name,
            heading = v.heading,
...
local function openFloorsMenu(lift, floor)
    ...
    exports['lrp-menu']:openMenu(floorsMenu)
end
...
local function changeFloor(data)
    ...
    local waittime = math.random(4000, 12000)

    QBCore.Functions.Progressbar("Call_The_Lift", Config.Language[Config.UseLanguage].Waiting, waittime, false, false, {
