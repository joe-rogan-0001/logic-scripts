$(document).ready(function() {
        ...
        window.addEventListener("message", function(event) {
            ...
            switch (data.type) {
                case 'open':
                    ...
                    break;
                case 'removeincoming':
                    $(".top-notifications-wrapper-mounted").empty();
                    break;
                case 'close':
            ...
            if (event.data.type === 'messageslog') {
                $('#listealmessage').prepend(createMessage(event.data.number, event.data.sonmsj, event.data.name));
                $("#" + event.data.number + "").click(function() {
                    ...
                    $.post('http://ld-phone/loadcontactmessages', JSON.stringify({
                        number: this.id
                    }));
                });
            }
            ...
            if (event.data.type === 'showmessages') {
                ...
                <li class="message message-out">
                    ...
                    <div class="timestamp timestamp-in">
                        <p class="MuiTypography-root timestamp MuiTypography-body2 MuiTypography-colorTextPrimary" style="word-break: break-word;">just now</p> 
                    </div>
                </li>
                `
    ...
    `);
        ...
        $('.anon-ping').click(function() {
            ...
            $.post('http://ld-phone/SendPing', JSON.stringify({
                "id": ping,
                "anon": true,
            }), function(data) {
                if (data) {
    ...
    function createMessage(number, sonmsg, name) {
        var html = "";
        html +=
            `
    ...
    function updateAdverts() {
        $.post('http://ld-phone/GetAdverts', function(adverts) {
    ...
    function updateTweets() {
        ...
        $(".jss2277").empty();
        $.post('http://ld-phone/GetTweets', function(tweets) {
            ...
            <div class="twat-low" style="margin-top: 8px;">
                ...
                <div class="twat-low-time">
                    <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary" style="word-break: break-word;">a few seconds ago</p>
                </div>
            </div>
            </div>`
    ...
    function createNotifyCallAction(title, detail) {
        ...
        if (PhoneData.Number != phoneNumber) {
            $.post('http://ld-phone/CallContact', JSON.stringify({
                number: phoneNumber
            }), function(callback) {
                if (callback) {
                    var htmlse = "";
    ...
    function incomingCall(detail) {
        ...
        $(document).ready(function() {
            ...
            $('.accept-call').click(function() {
                let testdoc = this
                PhoneData.InCall = true
                $.post('http://ld-phone/AcceptCall', JSON.stringify({
    ...
    function updateBankLogs() {
        ...
        $.post('http://ld-phone/GetBankLogs', function(data) {
    ...
    $(document).ready(function() {
        ...
        $('.wb-submit').click(function() {
            ...
            if (valueNo.length == 10 && valueAm.length >= 1 && valueCom.length >= 1) {
                $.post('http://ld-phone/SendMoneyToNumber', JSON.stringify({
                    number: valueNo,
                    amount: valueAm,
                    comment: valueCom
                }), function(data) {
    ...
    function updateContactList() {
        $.post('http://ld-phone/GetContacts', {}, function(data) {
            if (data) {
                $('.jss1481').html('');
                ...
                <div title="Copy Number" class="copy-number-c" number="${userNumber}">
                ...
                <path fill="currentColor" d="M384 112v352c0 26.51-21.49 48-48 48H48c-26.51 0-48-21.49-48-48V112c0-26.51 21.49-48 48-48h80c0-35.29 28.71-64 64-64s64 28.71 64 64h80c26.51 0 48 21.49 48 48zM192 40c-13.255 0-24 10.745-24 24s10.745 24 24 24 24-10.745 24-24-10.745-24-24-24m96 114v-20a6 6 0 0 0-6-6H102a6 6 0 0 0-6 6v20a6 6 0 0 0 6 6h180a6 6 0 0 0 6-6z">
                    ...
                    $("#remove-contact").click(function() {
                        let number = $(this).attr("number")
                        $.post('http://ld-phone/RemoveContact', JSON.stringify({
                            number: number
                        }), function(data) {
                            if (data) {
                                updateContactList();
    ...
    $(document).ready(function() {
        function createContract(name, phone) {
            $.post('http://ld-phone/AddContact', JSON.stringify({
                name: name,
                phone: phone
            }), function(data) {
                clearContractForm();
    ...
    function details() {
        ...
        <div class="" title="Bank Account ID">
        <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="university" class="svg-inline--fa fa-university fa-w-16 fa-fw fa-2x " role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
    ...
    function updateMails() {
        $('.jss1944').empty();
        $.post('http://ld-phone/GetPlayerMails', function(data) {
            if (data.length < 1) {
                var html = "";
                html +=
                    `
            ...
            style="word-break: break-word;">Nothing Here!</h6>`;
                ...
                $('.jss1944 .flex-centered').empty();
                $('.jss1944 .flex-centered').prepend(html);
            } else {

                $.each(data, function(__, mail) {
                    var subject = mail.subject
                    var name = mail.sender;
                    var date = new Date(mail.time.month + '/' + mail.time.day + '/' + mail.time.year)
    ...
    function abdu() {
        ...
        $('.jss179111').empty();
        $.post('http://ld-phone/GetEmployees', JSON.stringify({
            "job": "taxi"
        }), function(drivers) {
    ...
    function message() {
        $("#app-container").hide();
        $.post('http://ld-phone/messagesdatas', JSON.stringify({}));
    ...
    function mainmenu() {
        if ($("#app-container").is(":visible")) {
            ...
            $.post('http://ld-phone/kapatildi', JSON.stringify({}));
        }
    }
    ...
    $(document).on('keypress', function(e) {
        ...
        if (e.which === 13) {
            if (!sender == " ") {
                if (messagesacik) {
                    $.post('http://ld-phone/sendmessagebut', JSON.stringify({
                        number: karsinumber,
                        mesaj: sender
                    }));
                    setTimeout(function() {
    ...
    $(document).ready(function() {
        ...
        $('.submit-sari').click(function() {
            ...
            if (sariDetail.length >= 1 && sariName.length >= 1) {
                $.post('http://ld-phone/SendAdvert', JSON.stringify({
                    "name": sariName,
                    "detail": sariDetail
                }))
                complateInput();
    ...
    function createCamera() {
        $('.camera-inside').empty();
        $.post('http:/ld-phone/GetCameras', function(cameras) {
            if (cameras.length == 0) {
                let html = "";
                html += `
            ...
            <div camerahover=${random} class="component-paper camera-count" style="width: 100%;">
                ...
                $(".eye-camera").click(function() {
                    ...
                    $.post('http:/ld-phone/OpenCamera', JSON.stringify({
                        "name": cameraId
                    }));
                })
                $(".component-paper").mouseover(function() {
    ...
    $(document).ready(function() {
        ...
        $('.submit-twat').click(function() {
            if ($('.twat-message').val() == '') {
                ...
                $.post('http://ld-phone/SendTweet', JSON.stringify({
                    "message": msg,
                    "attachment": attach
                }))
                complateInputJustGreen();
    ...
    function createCar(value) {
        ...
        var status = value.state ? 'Available' : 'Not Available';
        var location = "City Hall";
        var html = "";
        html +=
        ...
        <div class="drawer">
            ...
            <div class="item" title="Engine">
                ...
                <div class="text">
                    <p class="MuiTypography-root MuiTypography-body2 MuiTypography-colorTextPrimary"
                    style="word-break: break-word;">100%</p>
                </div>
            </div>
    ...
    function createCarAction() {
        $(".jss2717").empty();
        $.post('http://ld-phone/GetPlayerVehicles', {}, function(data) {
            $.each(data, function(k, v) {
                ...
                $('.track-button').click(function(e) {
                    ...
                    $.post('http://ld-phone/SpawnVehicle', JSON.stringify({
                        model: modelName,
                        plate: carPlate,
                    }), function(data) {
                        if (data) {
                            ...
                            <path fill="currentColor" d="M499.99 176h-59.87l-16.64-41.6C406.38 91.63 365.57 64 319.5 64h-127c-46.06 0-86.88 27.63-103.99 70.4L71.87 176H12.01C4.2 176-1.53 183.34.37 190.91l6 24C7.7 220.25 12.5 224 18.01 224h20.07C24.65 235.73 16 252.78 16 272v48c0 16.12 6.16 30.67 16 41.93V416c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32v-32h256v32c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32v-54.07c9.84-11.25 16-25.8 16-41.93v-48c0-19.22-8.65-36.27-22.07-48H494c5.51 0 10.31-3.75 11.64-9.09l6-24c1.89-7.57-3.84-14.91-11.65-14.91zm-352.06-17.83c7.29-18.22 24.94-30.17 44.57-30.17h127c19.63 0 37.28 11.95 44.57 30.17L384 208H128l19.93-49.83zM96 319.8c-19.2 0-32-12.76-32-31.9S76.8 256 96 256s48 28.71 48 47.85-28.8 15.95-48 15.95zm320 0c-19.2 0-48 3.19-48-15.95S396.8 256 416 256s32 12.76 32 31.9-12.8 31.9-32 31.9z">
                                    ...
                                    "Vehicles", `Car is on the way!`, "just now", "linear-gradient(167deg, rgba(249,129,121,1) 0%, rgba(184,52,96,1) 100%)")
                            }, 1700);
                        } else {
                            createNotifyAction(`<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="car" class="svg-inline--fa fa-car fa-w-16 fa-fw fa-1x " role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                        <path fill="currentColor" d="M499.99 176h-59.87l-16.64-41.6C406.38 91.63 365.57 64 319.5 64h-127c-46.06 0-86.88 27.63-103.99 70.4L71.87 176H12.01C4.2 176-1.53 183.34.37 190.91l6 24C7.7 220.25 12.5 224 18.01 224h20.07C24.65 235.73 16 252.78 16 272v48c0 16.12 6.16 30.67 16 41.93V416c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32v-32h256v32c0 17.67 14.33 32 32 32h32c17.67 0 32-14.33 32-32v-54.07c9.84-11.25 16-25.8 16-41.93v-48c0-19.22-8.65-36.27-22.07-48H494c5.51 0 10.31-3.75 11.64-9.09l6-24c1.89-7.57-3.84-14.91-11.65-14.91zm-352.06-17.83c7.29-18.22 24.94-30.17 44.57-30.17h127c19.63 0 37.28 11.95 44.57 30.17L384 208H128l19.93-49.83zM96 319.8c-19.2 0-32-12.76-32-31.9S76.8 256 96 256s48 28.71 48 47.85-28.8 15.95-48 15.95zm320 0c-19.2 0-48 3.19-48-15.95S396.8 256 416 256s32 12.76 32 31.9-12.8 31.9-32 31.9z">
                                ...
                                "Vehicles", `Vehicle isn't in garage!`, "just now", "linear-gradient(167deg, rgba(249,129,121,1) 0%, rgba(184,52,96,1) 100%)")
                        }
                    })
                })
            })
    ...
    // Debt Functions
    function createDebt() {
        $.post('http://ld-phone/GetDebts', {}, function(data) {
            $(".jss271711").empty();
            if (data.length == 0) {
                let html = "";
                html += `
            ...
            $.each(data, function(__, debt) {
                ...
                $(".pawynow-button").click(function(e) {
                    ...
                    $.post('http://ld-phone/PayInvoice', JSON.stringify({
                        "id": debtId
                    }), function(data) {
                        if (data.status == true) {
                            $("#debt-" + debtId).remove();
    ...
    $(document).ready(function() {
        function createdoj() {
            $.post('http://ld-phone/GetCases', function(data) {
                if (data.length == 0) {
                    let html = "";
                    html += `
            <div class="flex-centered" style="padding: 32px; flex-direction: column; text-align: center;">
    ...
    $(document).ready(function() {
        function createDiamond() {
            $.post('http://ld-phone/GetEvents', JSON.stringify({}), function(events) {
    ...
    $('#togglesounds').click(function() {
        ...
        $.post('http://ld-phone/ToggleSound')
    })

    $("input[type=text], textarea, input[type=number]").focusin(function() {
        $.post("http://ld-phone/LockKeyboard", JSON.stringify({}));
    });

    $("input[type=text], textarea, input[type=number]").focusout(function() {
        $.post("http://ld-phone/ReleaseKeyboard", JSON.stringify({}));
    });
