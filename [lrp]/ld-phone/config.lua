Config = {
    ...
    GetSharedObject = function(cb)
        cb(exports['lrp-core']:GetCoreObject())
    ...
    PlayerLoad = "QBCore:Client:OnPlayerLoaded", -- esx:playerLoaded
    PlayerFunctions = {
        addAccountMoney = function(player, account, amount)
            player.Functions.AddMoney(account, amount)
        end,
        removeAccountMoney = function(player, account, amount)
    ...
    Details = {
        Casino = {
            ...
            CasinoMoneyFunction = function(Framework, source)
                ...
                return xPlayer.PlayerData.money["casino"]
            end,
        },
        Bank = {
    ...
    DatabaseStrings = {
        ["executeSync"] = function(string, table)
            table = table or {}
            return exports["oxmysql"]:executeSync(string, table)
        end,
        ["execute"] = function(string, table, callback)
            table = table or {}
            exports["oxmysql"]:execute(string, table, function(rowsChanged)
    ...
    ItemCheck = function(Framework, callback)
        Framework.Functions.TriggerCallback('ld-phone:check-phone', function(state) 
            callback(state)
        end)
    end,
    ...
    Applications = {
        ...
        ShowNotAvailableApps = {
            enable = false, -- if enable = false, then the app will not be shown. (btw you make this option true not available apps will be shown but didnt be useable.) 
    ...
    }, -- You can edit notify SVG's from there..

    PlayerBillingSettings = {
        DataTableName = "phone_invoices",
        IdentifierColumnName = "citizenid",
        IDColumnName = "id",
    },

    PlayerVehiclesSettings = {
        DataTableName = "player_vehicles",
        IdentifierColumnName = "citizenid",
        StoredColumnName = "stored",
    ...
    UsePlayerLicenses = false, -- Use license system in details
