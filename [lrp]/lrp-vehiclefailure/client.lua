...
RegisterNetEvent('qb-vehiclefailure:client:RepairVehicle', function()
	...
	if vehicle and vehicle > 0 and engineHealth < 500 then
		local ped = PlayerPedId()
		local pos = GetEntityCoords(ped)
...
RegisterNetEvent('iens:repaira2', function()
	local ped = PlayerPedId()
	local pos = GetEntityCoords(ped)
	vehicle = QBCore.Functions.GetClosestVehicle(vector3(pos.x, pos.y, pos.z))
	if vehicle ~= 0 then
		if #(pos - GetEntityCoords(vehicle)) <= 3.0 then
			SetVehicleDirtLevel(vehicle)
			SetVehicleUndriveable(vehicle, false)
			WashDecalsFromVehicle(vehicle, 1.0)
			notification("Vehicle repaired!")
			...
			notification("No nearby vehicle!")
		end
	end
end)
