...
RegisterNetEvent('weapons:client:SetWeaponQuality', function()
    if CurrentWeaponData and next(CurrentWeaponData) then
        if CurrentWeaponData.info.repamo == "3" then
            QBCore.Functions.Notify("Weapon cannot be repaired anymore!", "error")   
        else 
            loadAnimDict("mini@repair")
            TaskPlayAnim(PlayerPedId(), "mini@repair", "fixing_a_ped" ,8.0, 8.0, -1, 81, 0, 0, 0, 0)

            QBCore.Functions.Progressbar("taking_bullets2", "Reparing Weapon", math.random(15000, 20000), false, false, {
                ...
                if CurrentWeaponData.info.repamo == null then 
                    TriggerServerEvent("weapons:server:SetWeaponQuality", CurrentWeaponData, "1", 90)
                elseif CurrentWeaponData.info.repamo == "1" then
                    TriggerServerEvent("weapons:server:SetWeaponQuality", CurrentWeaponData, "2", 70) 
                elseif CurrentWeaponData.info.repamo == "2" then
                    TriggerServerEvent("weapons:server:SetWeaponQuality", CurrentWeaponData, "3", 50)
                end
            end, function()
...
RegisterNetEvent('weapons:client:scratchweaponserial', function()
    if CurrentWeaponData and next(CurrentWeaponData) then
        if CurrentWeaponData.info.serie == "Serial Scratched" then
            QBCore.Functions.Notify("Weapon serial already scratched!", "error")   
        else 
            loadAnimDict("mini@repair")
            TaskPlayAnim(PlayerPedId(), "mini@repair", "fixing_a_ped" ,8.0, 8.0, -1, 81, 0, 0, 0, 0)

            QBCore.Functions.Progressbar("taking_bullets4", "Scratching Weapon Serial", math.random(15000, 20000), false, false, {
                disableMovement = false,
                disableCarMovement = false,
                disableMouse = false,
                disableCombat = true,
                ...
                ClearPedTasksImmediately(PlayerPedId())
                TriggerServerEvent("weapons:server:scratchweaponserial", CurrentWeaponData)
            end, function()
                QBCore.Functions.Notify("Canceled", "error")
            end)  
        end
...
RegisterNetEvent('weapon:client:AddAmmo', function(type, amount, itemData)
    local ped = PlayerPedId()
    local weapon = GetSelectedPedWeapon(ped)
    if (itemData.name == "hunting_ammo") then 
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_sniperrifle2" then
            return 
        end
    end
