...
$(document).on('click', '#save', function(){
    $.post('https://lrp-tunerchip/save', JSON.stringify({
...
$(document).on('click', '#reset', function(){
    $.post('https://lrp-tunerchip/reset');
});
...
$(document).on('click', "#save-neon", function(){
    if (RainbowNeon) {
        $.post('https://lrp-tunerchip/saveNeon', JSON.stringify({
...
$(document).on('click', '#save-headlights', function(){
    $.post('https://lrp-tunerchip/saveHeadlights', JSON.stringify({
        value: headlightVal,
...
$(document).on('click', '#save-stancer', function(){
    ...
    $.post('https://lrp-tunerchip/SetStancer', JSON.stringify({
...
$(document).on('click', ".neon-software-color-pallete-color", function(){
    ...
    if (!$(this).data('rainbow')) {
        ...
        if (headlightValue != null) {
            ...
            var colorValue1 = $(this).css("background");
            if (headlightVal === 'rainbow') {
                var colorValue1 = $(this).css("background");
                $(".neon-software-color-pallete-color-current").css("background", colorValue1);
            } else {
                $(".neon-software-color-pallete-color-current").css("background", colorValue1);
...
QBTuner.Open = function() {
    $.post('https://lrp-tunerchip/checkItem', JSON.stringify({item: "tunerlaptop"}), function(hasItem){
        if (hasItem) {
