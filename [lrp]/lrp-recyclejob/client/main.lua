...
RegisterNetEvent("recyclejob:client:duty", function()
    onDuty = not onDuty
    if onDuty then
      QBCore.Functions.Notify("You Have Been Clocked In", "success")
    else
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) < 2.3 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            QBCore.Functions.Progressbar("pickup_reycle_package", "Picking Up Package..", math.random(8000, 12000), false, true, {
                    ...
                    if #(pos - vector3(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z)) < 2.0 then
                        ...
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Unpacking Package..", math.random(8000, 10000), false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
...
exports['lrp-target']:AddBoxZone("recyclejobduty", vector3(995.15, -3099.97, -39.18), 1.00, 1.00, {
    name = "recyclejobduty",
    heading = 269.53,
    debugPoly = false,
    minZ = -39.50,
