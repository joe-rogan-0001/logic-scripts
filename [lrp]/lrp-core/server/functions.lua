...
function PaycheckLoop()
    ...
    for _, Player in pairs(Players) do
        local pAmount = Player.PlayerData.job.payment
        if Player.PlayerData.job and pAmount > 0 and (QBShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
            --if QBCore.Config.Money.PayCheckSociety then

                local pSource = Player.PlayerData.source
                print("paid "..pSource.." "..pAmount)
                TriggerEvent('qb-banking:server:sendPaycheck', pAmount, pSource)
