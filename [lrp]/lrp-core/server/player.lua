...
function QBCore.Player.Login(source, citizenid, newData)
    ...
    if src then
        if citizenid then
            ...
            local PlayerData = MySQL.Sync.fetchSingle('SELECT * FROM players where citizenid = ?', { citizenid })
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.job.payment = PlayerData.job.payment or 150
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        TriggerClientEvent('lrp-armourupdate', src)
...
QBCore.Player.LoadInventory = function(PlayerData)
    PlayerData.items = {}
    local inventory = MySQL.Sync.fetchScalar('SELECT inventory FROM players WHERE citizenid = ?', { PlayerData.citizenid })
    if inventory then
...
function QBCore.Player.CreateCitizenId()
    ...
    while not UniqueFound do
        ...
        local result = MySQL.Sync.fetchScalar('SELECT COUNT(*) as count FROM players WHERE citizenid = ?', { CitizenId })
        if result == 0 then
            UniqueFound = true
        end
    end
...
function QBCore.Functions.CreateAccountNumber()
    ...
    while not UniqueFound do
        AccountNumber = math.random(111111, 999999)
...
function QBCore.Player.CreateFingerId()
    ...
    while not UniqueFound do
        ...
        local result = MySQL.Sync.fetchScalar('SELECT COUNT(*) as count FROM `players` WHERE `metadata` LIKE ?', { query })
        if result == 0 then
            UniqueFound = true
        end
    end
