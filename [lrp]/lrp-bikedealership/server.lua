...
RegisterNetEvent('qb-bikedealership:server:addPlayer', function(citizenid, gameTime)
    financetimer[citizenid] = gameTime
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('qb-bikedealership:server:removePlayer', function(citizenid)
...
-- Callbacks
QBCore.Functions.CreateCallback('qb-bikedealership:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('qb-bikedealership:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('qb-bikedealership:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('qb-bikedealership:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('qb-bikedealership:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('qb-bikedealership:client:customTestDrive', target, vehicle)
    else
...
RegisterNetEvent('qb-bikedealership:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-bikedealership:server:financePaymentFull', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('qb-bikedealership:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > vehiclePrice then
        ...
        TriggerClientEvent('qb-bikedealership:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('qb-bikedealership:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
...
RegisterNetEvent('qb-bikedealership:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('qb-bikedealership:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
...
RegisterNetEvent('qb-bikedealership:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local target = QBCore.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('qb-bikedealership:server:checkFinance', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
