...
RegisterNetEvent('qb-bankrobbery:server:BuyLaptop', function(index)
    ...
    if Player.Functions.GetItemByName(item) then
        if (Player.PlayerData.money.rook - Laptops[index].price) >= 0 then
            -- Remove USB
            Player.Functions.RemoveItem(item, 1)
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item], "remove")
            -- Remove rook
            Player.Functions.RemoveMoney('rook', Laptops[index].price)
            -- Add Laptop
            ...
            if debug then
                print('^3[qb-bankrobbery] ^5'..GetPlayerName(src)..'(citizenid: '..Player.PlayerData.citizenid..' | id: '..src..') Purchased '..laptop..' for '..Laptops[index].price..' rook^7')
            end
            TriggerEvent("qb-log:server:CreateLog", "bankrobbery", "Purchased "..laptop, "white", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) purchased one "..laptop.." from "..Laptops[index].name.." for "..Laptops[index].price.." rook")
            TriggerClientEvent('QBCore:Notify', src, "You purchased a laptop from "..Laptops[index].name.." for "..Laptops[index].price.." rook", "success", 2500)
        else
            TriggerClientEvent('QBCore:Notify', src, "You don't have enough rook", "error", 2500)
        end
    else
...
RegisterNetEvent('qb-bankrobbery:server:BuyGreyUsb', function()
    ...
    local price = 2.30
    local info = {uses = 3}
    if (Player.PlayerData.money.rook - price) >= 0 then
        Player.Functions.RemoveMoney('rook', price)
        ...
        if debug then
            print('^3[qb-bankrobbery] ^5'..GetPlayerName(src)..'(citizenid: '..Player.PlayerData.citizenid..' | id: '..src..') Purchased Grey USB for '..price..' rook^7')
        end
        TriggerEvent("qb-log:server:CreateLog", "bankrobbery", "Purchased Grey USB", "white", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) purchased one usb_grey from Daut for "..price.." rook")
        TriggerClientEvent('QBCore:Notify', src, "You purchased a laptop from Daut for "..price.." rook", "success", 2500)
    else
        TriggerClientEvent('QBCore:Notify', src, "You don't have enough rook", "error", 2500)
    end
end)
