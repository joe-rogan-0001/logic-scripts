...
var ShowroomFunctions = {
    ...
    GetCars: function () {
        $.post('https://lrp-cardealer/lrp-ui:showroomChangeCar', JSON.stringify({ model: cars[selectedId].model }), function (cb) {
...
$(document).ready(function () {
    ...
    window.addEventListener('message', (event) => {
        ...
        if (data.meta.message = "done") {
            ...
            $.post('https://lrp-cardealer/lrp-ui:application-closed', JSON.stringify({ name: "showroom" }));
        }
    })
});
...
function myFunction() {
    canPress = false
    $.post('https://lrp-cardealer/showroomPurchaseCurrentVehicle', JSON.stringify({ model: cars[selectedId].model, price: cars[selectedId].price, zoneName: zone }), function(cb){
     if( cb.meta.ok) {
        ShowroomFunctions.Close()
        canPress = false
     } else {
