	...
	methods: {
		...
		spawn() { 
			...
			sendNuiMessage({
				...
				//moreadata : this.currentCharData
			})
		},
...
function receivedNuiMessage(event) {
	...
	if (data.update){
		if (data.currentSelect != null){
			...
			//Hover on/off
			//console.log(data.currentSelect.position)
			...
			//console.table(JSON.stringify(v.$data.currentCharData))
			...
			if (v.$data.currentCharData != null){
				...
				charinfo = JSON.parse((v.$data.currentCharData.charinfo))

				//Check if name length > 20 chars
				v.$data.longName = ((charinfo.firstname).length + (charinfo.lastname).length > 20);
			}
		}else{
...
function sendNuiMessage(data){
	$.post("https://lrp-login/nuiCallback", JSON.stringify(data));
}
