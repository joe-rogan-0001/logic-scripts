...
AddEventHandler('spawn:clientSpawnData', function(spawnData)
	...
	currentSpawns[#currentSpawns + 1] = Spawn.getGangSpawn()
	currentSpawns[#currentSpawns + 1] = Spawn.motel[1]

	apartmentid = spawnData["motelRoom"].roomnumber
...
function Spawn.selectedSpawn(spawnInfo)
	...
	TriggerServerEvent('iconic-clothingmenu:get_character_current')
	TriggerEvent("iconic-clothingmenu:Spawning", false)
	...
	if apartment then
		DoScreenFadeOut(2)		
		TriggerEvent('qb-apartments:client:LastLocationHouse', 'apartment1', apartmentid)
	else
		...
		if pos then
			...
			--print(dump(apartment.motel["motelRoom"]["cid"]))
...
function Spawn.overwriteSpawn(overwrite, data)
	...
	if overwrite == "jail" then
		SetEntityCoordsNoOffset(PlayerPedId(), 1802.51,2607.19,46.01, false, false, false, true)
		TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
		TriggerEvent('QBCore:Client:OnPlayerLoaded')
