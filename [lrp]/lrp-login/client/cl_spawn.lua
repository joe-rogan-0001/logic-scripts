...
function Login.nativeStart(AlreadySpawned)
	...
	if not AlreadySpawned then
		tatCategory = exports["lrp-clothingmenu"]:GetTatCategs()
		tattooHashList = exports["lrp-clothingmenu"]:GetTatCategs()
		Login.custompeds = exports["lrp-clothingmenu"]:GetCustomSkins()
...
RegisterNUICallback("nuiCallback", function(data, callback)
	...
	if data.action == "singleClick" or data.action == "doubleClick" then
	    local offset = vector2(data.x,data.y)
	    local coords = QBCore.Functions.ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset) --HERE
	    local pedCaught = LocationInWorld(coords,LoginSafe.Cam)
	   	local pedData = nil
	    if data.safe then
	...
	if data.action == "deleteCharacter" then
		...
		QBCore.Functions.TriggerCallback("lrp-spawn:server:deletecharacter", function(result) 
			if result then
				Login.actionsBlocked = false
				Login.getCharacters(true)
			else	
				print("Issue detected while deleting your character. Contact an admin.")
			end
		end, data.actionData)
	end
	...
	if data.action == "newCharacter" then
		...
		local chardata = {
			...
			nationality = "American", -- fix this shit
			birthdate = cData.dob,
			gender = gender,
		...
		TriggerServerEvent('mr-characters:server:createCharacter', chardata)
...
RegisterNetEvent("np-base:spawnInitialized", function()
	Login.nativeStart()
end)
