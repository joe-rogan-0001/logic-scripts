...
CreateThread(function()
	...
	Targets["JimBank"] =
	exports['lrp-target']:AddCircleZone("JimBank", vector3(Config.CashInLocation.x, Config.CashInLocation.y, Config.CashInLocation.z), 2.0, { name="JimBank", debugPoly=Config.Debug, useZ=true, },
		{ options = {
	...
	for k, v in pairs(Config.CustomCashRegisters) do
		for i = 1, #v do
			...
			exports['lrp-target']:AddBoxZone("CustomRegister: "..k..i, v[i].coords.xyz, 0.47, 0.34, { name="CustomRegister: "..k..i, heading = v[i].coords[4], debugPoly=Config.Debug, minZ=v[i].coords.z-0.1, maxZ=v[i].coords.z+0.4 },
...
RegisterNetEvent('jim-payments:client:Charge', function(data, outside)
	...
	local newinputs = {} -- Begin lrp-input creation here.
	if Config.List then -- If nearby player list is wanted:
		...
		local nearbyList = {}
		--Convert list of players nearby into one lrp-input understands + add distance info
	...
	--Continue adding payment options to lrp-input
	...
	local dialog = exports['lrp-input']:ShowInput({ header = img..label..Loc[Config.Lan].menu["cash_reg"], submitText = Loc[Config.Lan].menu["send"], inputs = newinputs})
	if dialog then
...
RegisterNetEvent('jim-payments:client:PolCharge', function()
	...
	local label = PlayerJob.label
	local gang = false
	local dialog = exports['lrp-input']:ShowInput({ header = label..Loc[Config.Lan].menu["charge"], submitText = Loc[Config.Lan].menu["send"], inputs = newinputs})
	if dialog then
		if not dialog.citizen or not dialog.price then return end
...
RegisterNetEvent('jim-payments:Tickets:Sell:no', function() exports['lrp-menu']:closeMenu() end)
