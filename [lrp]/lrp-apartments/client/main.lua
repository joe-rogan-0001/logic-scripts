...
local ClosestHouse, CurrentApartment, rangDoorbell, POIOffsets
local IsOwned = false
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                data = exports['lrp-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                houseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
...
local exittext = true
local stashtext = true
local outfittext = true
local logouttext = true
local entertext = true
