...
-- UPDATE TO DO BASED ON RANK WITH FOR LOOP
QBCore.Commands.Add("raidapartment", "raid persons apartment", {{name = "cid", help = "CitizenID of person"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.grade.level >= 7 then
        if args[1] then
            local result = MySQL.Sync.fetchAll('SELECT * FROM apartments WHERE citizenid = ?', {args[1]})
            if result[1] then
                TriggerClientEvent("apartments:client:SpawnInApartment", src, result[1].name, 'apartment1')
            else
                TriggerClientEvent('QBCore:Notify', src, "The person you are trying to raid does not own an apartment", "error")
            end
        else
            TriggerClientEvent('QBCore:Notify', src, "Who you trying to raid?", "error")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "You must be High Command or above to grant licenses!", "error")
    end
end)