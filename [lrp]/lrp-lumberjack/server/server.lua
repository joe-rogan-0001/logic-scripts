...
local Chopped = false
local amounts = LumberJob.TradeAmount



QBCore.Functions.CreateCallback('lrp-lumberjack:server:amounts', function(source, cb)
    local src = source
...
RegisterServerEvent('tr-lumberjack:recivelumber', function()
    ...
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['tree_lumber'], "add", lumber)
...
RegisterServerEvent('tr-lumberjack:lumberprocessed', function(tradeamount)
    ...
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['tree_lumber'], "remove", amount)
    TriggerClientEvent('QBCore:Notify', source, 'You successfully traded ' ..amount.. ' lumber for ' ..LumberJob.TradeReceived.. ' wooden planks')
    local amount = LumberJob.TradeReceived
    Wait(750)
    Player.Functions.AddItem('wood_plank', amount)
    TriggerClientEvent('inventory:client:ItemBox', source, QBCore.Shared.Items['wood_plank'], "add", amount)
end)