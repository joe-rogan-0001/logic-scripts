	...
	while true do
		...
		for k, v in pairs(Config.GaragePedLocations) do
			...
			if dist < 40 and not pedspawned then
				TriggerEvent('lrp-burgershot:spawn:ped', v.coords)
				pedspawned = true
			end
			if dist >= 35 then
				pedspawned = false
...
RegisterNetEvent('lrp-burgershot:spawn:ped')
AddEventHandler('lrp-burgershot:spawn:ped',function(coords)
	local hash = `ig_floyd`
...
RegisterNetEvent('lrp-burgershot:garage')
AddEventHandler('lrp-burgershot:garage', function(bs)
...
RegisterNetEvent('lrp-burgershot:storecar')
AddEventHandler('lrp-burgershot:storecar', function()
