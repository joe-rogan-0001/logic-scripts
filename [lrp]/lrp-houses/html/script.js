...
$('document').ready(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 116 ) {
            $.post('https://lrp-houses/toggleCursor');
        }

        if (data.which == 13 ) {
            if (selectedObjectData != null && $(".decorate-confirm").css('display') != 'block') {
                $.post('https://lrp-houses/editOwnedObject', JSON.stringify({
                    objectData: selectedObjectData
                }));
                selectedObjectData = null;
                $.post('https://lrp-houses/setupMyObjects', JSON.stringify({}), function(myObjects){
                    $('.decorate-items').html("");
                ...
                if (objData != null) {
                    $.post("https://lrp-houses/spawnobject", JSON.stringify({
...
$(document).on('click', '.header-btn', function(){
    ...
    $.post('https://lrp-houses/ResetSelectedProp');
...
$(document).on('click', '.footer-btn', function(){
    ...
    if (selectedCategory != "remove-owned-obj") {
        ...
        $(".decorate-footer-buttons").html("");
        $(selectedHeaderButton).removeClass('header-btn-selected');
        $.post('https://lrp-houses/deleteSelectedObject');
        $(".decorate-footer-buttons").fadeOut(150);
        $(".decorate-items").fadeOut(150);
    }
});
...
$(document).on('click', '#buy-object', function(){
    $.post("https://lrp-houses/buySelectedObject", JSON.stringify({
...
$(document).on('click', '#cancel-object', function(){
    $.post('https://lrp-houses/cancelSelectedObject');
    selectedObjectData = null;
    $(".decorate-confirm").css("display", "none");
});
...
$(document).on('click', '.decorate-item', function(){
    ...
    if ($("#"+objId).data('type') == "newObject") {
        if (selectedObject == this) {
            ...
            selectedObject = null;
            $.post('https://lrp-houses/removeObject');
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            $.post("https://lrp-houses/chooseobject", JSON.stringify({
                object: objData.object,
            }));
        ...
        if (selectedObject == this) {
            $(this).removeClass('selected-object');
            $.post('https://lrp-houses/deselectOwnedObject')
            selectedObject = null;
        } else {
            $(this).addClass('selected-object');
            selectedObject = this;
            selectedObjectData = myObjectData;
            $.post('https://lrp-houses/selectOwnedObject', JSON.stringify({
...
Decorations.Close = function() {
    ...
    selectedObjectData = null;
    $.post("https://lrp-houses/closedecorations", JSON.stringify({}));
}

$(".property-accept").click(function(e){
    $.post('https://lrp-houses/buy', JSON.stringify({}))
});

$(".property-cancel").click(function(e){
    $.post('https://lrp-houses/exit', JSON.stringify({}));
});
