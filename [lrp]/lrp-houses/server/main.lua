...
RegisterNetEvent('qb-houses:server:viewHouse', function(data)
    local house = data.closesthouse
    local src = source
    --print(src)
...
RegisterNetEvent('qb-houses:server:giveKey', function(house, target)
    ...
    MySQL.Async.fetchAll('UPDATE player_houses SET keyholders = ? WHERE house = ?',
...
RegisterNetEvent('qb-houses:server:removeHouseKey', function(house, citizenData)
    ...
    MySQL.Async.fetchAll('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
end)
...
RegisterNetEvent('qb-houses:server:savedecorations', function(house, decorations)
    MySQL.Async.fetchAll('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
...
RegisterNetEvent('qb-houses:server:LogoutLocation', function()
    ...
    MySQL.Async.fetchAll('UPDATE players SET inventory = ? WHERE citizenid = ?',
...
RegisterNetEvent('qb-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
        MySQL.Async.fetchAll('UPDATE player_houses SET stash = ? WHERE house = ?', {json.encode(coords), house})
    elseif type == 2 then
        MySQL.Async.fetchAll('UPDATE player_houses SET outfit = ? WHERE house = ?', {json.encode(coords), house})
    elseif type == 3 then
        MySQL.Async.fetchAll('UPDATE player_houses SET logout = ? WHERE house = ?', {json.encode(coords), house})
    end
