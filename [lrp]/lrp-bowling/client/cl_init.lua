...
Citizen.CreateThread(function()
    for k, v in pairs(lanes) do
        if (v.enabled) then 
        exports["polyzonehelper"]:AddBoxZone("bp-bowling:lane_"..k, v.pos, 1.8, 2.0, {
            heading=0,
            minZ=23.85,
            maxZ=27.85
        })
    ...
    exports["polyzonehelper"]:AddBoxZone("bowling_alley", vector3(743.95, -774.54, 26.34), 16.8, 30.4, {
        heading=0.0,
        minZ=23.85,
        maxZ=28.85,
        debugpoly = false
    ...
    exports['lrp-target']:AddTargetModel(BowlingPed, {
        options = {
            {
...
AddEventHandler('bp-bowling:client:openMenu' , function()
    ...
    exports['lrp-menu']:openMenu(uNwinDTestMenu)
end)
...
AddEventHandler('bp-bowling:openMenu2' , function(data)
    if(data.data == 'bowlingreceipt') then
        ...
        for k, v in ipairs(lanes) do
            ...
            exports['lrp-menu']:openMenu(uNwinDTestMenu2)
        end
...
function shit(k,v) 
    Citizen.CreateThread(function()
        while sheesh == true do
            exports['lrp-target']:AddBoxZone("bp-bowling:lane_"..k, v.pos, 1.8, 2.0, {
...
AddEventHandler("bp-bowling:bowlingPurchase", function(data)
    ...
    QBCore.Functions.TriggerCallback('bp-bowling:purchaseItem', function(response)
        if response then
            if(isLane) then
                for k, v in pairs(lanes) do
                    if (lanes[data.key].enabled) and (k == data.key) then
                        sheesh = true
                        shit(k , v)
                    end
                end
...
AddEventHandler("polyzonehelper:exit", function(zone)
    ...
    inBowlingZone = false
    --TriggerServerEvent("bp-bowling:RemoveItem") -- what is the point of this
