...
RegisterNetEvent('iconic-eblips:server:registerSourceName')
AddEventHandler('iconic-eblips:server:registerSourceName', function(source, name)
    if not source_cache[source] then source_cache[source] = { } end
    source_cache[source].name = name
end)

RegisterNetEvent('iconic-eblips:server:registerPlayerBlipGroup')
AddEventHandler('iconic-eblips:server:registerPlayerBlipGroup', function(source, group)
...
RegisterNetEvent('iconic-eblips:server:removePlayerBlipGroup')
AddEventHandler('iconic-eblips:server:removePlayerBlipGroup', function(source, group)
...
RegisterNetEvent('iconic-eblips:server:registerPlayerTempBlipGroup')
AddEventHandler('iconic-eblips:server:registerPlayerTempBlipGroup', function(source, group, time)
    TriggerEvent('iconic-eblips:server:registerPlayerBlipGroup', source, 'police')
    SetTimeout(time, function()
        TriggerEvent('iconic-eblips:server:removePlayerBlipGroup', source, 'police')
    end)
end)
            ...
            Citizen.CreateThread(function()
                for source, is_valid_member in pairs(blip.members) do
                    if is_valid_member then -- more sanity checking
                        TriggerClientEvent('iconic-eblips:client:syncMyBlips', source, blips_data)
                        Citizen.Wait(100)
                    end
                end
            end)
