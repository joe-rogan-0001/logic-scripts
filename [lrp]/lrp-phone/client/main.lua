...
RegisterCommand('phonetwttest', function()
    TriggerServerEvent('qb-phone:server:sendNewMail', {
        sender = 'Billing Department',
        subject = 'Invoice Paid',
        message = 'Invoice Has Been Paid From '..'hi'..' In The Amount Of $'..'hi',
    })
end)
...
RegisterNUICallback('pingSend', function(data, cb)
    if not data.number then
        ...
        TriggerServerEvent("iconic-ping:attempt",  GetEntityCoords(PlayerPedId()), data.number)
    end
end)

RegisterNUICallback('SetupInfo', function(data, cb)
    cb(QBCore.Functions.GetPlayerData())
end)
...
RegisterNUICallback("TakePhotoButton", function(data,cb)
    SetNuiFocus(false, false)
    CreateMobilePhone(1)
    CellCamActivate(true, true)
    takePhoto = true
            ...
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    QBCore.Functions.TriggerCallback('qb-phone:server:HasPhone', function(HasPhone)
                        if HasPhone then
                            RepeatCount = RepeatCount + 1
                            TriggerServerEvent("InteractSound_SV:PlayOnSource", "phonecall", 0.3)
    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                QBCore.Functions.Notify('Ping From ' .. pendingPing.name .. ' Timed Out', "success")
                TriggerServerEvent('iconic-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('iconic-ping:client:SendPing')
AddEventHandler('iconic-ping:client:SendPing', function(sender, senderId, pCoords)
    if pendingPing == nil then
        ...
        TriggerServerEvent('iconic-ping:server:SendPingResult', pendingPing.id, 'received')
        QBCore.Functions.Notify(pendingPing.name .. ' Sent you a ping, use /ping accept to accept', "success")
        isPending = true
        ...
        QBCore.Functions.Notify(sender .. ' Attempted To Ping You', "success")
        TriggerServerEvent('iconic-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('iconic-ping:client:AcceptPing')
AddEventHandler('iconic-ping:client:AcceptPing', function()
    if isPending then
        ...
        QBCore.Functions.Notify(pendingPing.name .. '\'s Location Showing On Map', "success")
        TriggerServerEvent('iconic-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
        QBCore.Functions.Notify('You Have No Pending Ping', "error")
    end
end)

RegisterNetEvent('iconic-ping:client:RejectPing')
AddEventHandler('iconic-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        QBCore.Functions.Notify('Rejected Ping From ' .. pendingPing.name, "error")
        TriggerServerEvent('iconic-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        QBCore.Functions.Notify('You Have No Pending Ping', "error")
...
RegisterNetEvent('iconic-ping:client:RemovePing')
AddEventHandler('iconic-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
        QBCore.Functions.Notify('Player Ping Removed', "info")
    else
        QBCore.Functions.Notify('You Have No Pending Ping', "error")
    end
end)
...
RegisterCommand('ping', function(src, args, raw)
    TriggerServerEvent("iconic-ping:attempt",  GetEntityCoords(PlayerPedId()), args[1])
end)
...
local function PublicPhone()
    local PublicPhoneobject = {
        ...
        exports["lrp-target"]:AddTargetModel(PublicPhoneobject, {
            options = {
                {
                    type = "client",
