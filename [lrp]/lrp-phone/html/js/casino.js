...
function LoadCasinoJob(){
    ...
    $.post('https://lrp-phone/CheckHasBetTable', JSON.stringify({}), function(HasTable){
...
$(document).on('click', '#casino_delete', function(e){
    e.preventDefault();
    $.post('https://lrp-phone/CasinoDeleteTable', JSON.stringify({}));
});

$(document).on('click', '#casino_status', function(e){
    e.preventDefault();
    $.post('https://lrp-phone/casino_status', JSON.stringify({}));
    CheckStatus();
});
...
$(document).on('click', '#casino-submit-bet', function(e){
    ...
    if (InName != "" && InChanse != "" && InChanse >= 1.0){
        $.post('https://lrp-phone/CasinoAddBet', JSON.stringify({
            name: InName,
            chanse: InChanse,
        }));
        ClearInputNew();
...
function CheckStatus(){
    $.post('https://lrp-phone/CheckHasBetStatus', JSON.stringify({}), function(HasStatus){
        if (HasStatus){
...
$(document).on('click', '#casino-end-task-accept', function(e){
    ...
    if (Amount != "" && Amount >= 1.0){
        $.post('https://lrp-phone/BettingAddToTable', JSON.stringify({
            amount: Amount,
...
$(document).on('click', '#casino-Winer-this', function(e){
    e.preventDefault();
        $.post('https://lrp-phone/WineridCasino', JSON.stringify({
            id: PlayerIDOf,
        }));
});