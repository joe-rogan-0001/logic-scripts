        ...
        if (CanOpenApp) {
            if (QB.Phone.Data.currentApplication == null) {
                ...
                if (PressedApplication == "settings") {
                    ...
                    $.post('https://lrp-phone/GetHashtags', JSON.stringify({}), function(Hashtags){
                    ...
                    $.post('https://lrp-phone/GetBankContacts', JSON.stringify({}), function(contacts){
                        QB.Phone.Functions.LoadContactsWithNumber(contacts);
                    });
                    $.post('https://lrp-phone/GetInvoices', JSON.stringify({}), function(invoices){
                        QB.Phone.Functions.LoadBankInvoices(invoices);
                    });
                } else if (PressedApplication == "whatsapp") {
                    $.post('https://lrp-phone/GetWhatsappChats', JSON.stringify({}), function(chats){
                        QB.Phone.Functions.LoadWhatsappChats(chats);
                    });
                } else if (PressedApplication == "phone") {
                    $.post('https://lrp-phone/GetMissedCalls', JSON.stringify({}), function(recent){
                        QB.Phone.Functions.SetupRecentCalls(recent);
                    });
                    $.post('https://lrp-phone/GetSuggestedContacts', JSON.stringify({}), function(suggested){
                        QB.Phone.Functions.SetupSuggestedContacts(suggested);
                    });
                    $.post('https://lrp-phone/ClearGeneralAlerts', JSON.stringify({
                        app: "phone"
                    }));
                } else if (PressedApplication == "mail") {
                    $.post('https://lrp-phone/GetMails', JSON.stringify({}), function(mails){
                    ...
                    $.post('https://lrp-phone/LoadAdverts', JSON.stringify({}), function(Adverts){
                        QB.Phone.Functions.RefreshAdverts(Adverts);
                    })
                } else if (PressedApplication == "garage") {
                    $.post('https://lrp-phone/SetupGarageVehicles', JSON.stringify({}), function(Vehicles){
                        SetupGarageVehicles(Vehicles);
                    })
                } else if (PressedApplication == "info") {
                    $.post('https://lrp-phone/SetupInfo', JSON.stringify({}), function(data){
                        openInfoApp(data);
                    })
                } else if (PressedApplication == "crypto") {
                    $.post('https://lrp-phone/GetCryptoData', JSON.stringify({
                        crypto: "qbit",
                    }), function(CryptoData){
                        SetupCryptoData(CryptoData);
                    })

                    $.post('https://lrp-phone/GetCryptoTransactions', JSON.stringify({}), function(data){
                        RefreshCryptoTransactions(data);
                    })
                } else if (PressedApplication == "racing") {
                    $.post('https://lrp-phone/GetAvailableRaces', JSON.stringify({}), function(Races){
                        SetupRaces(Races);
                    });                
                } else if (PressedApplication == "houses") {
                    $.post('https://lrp-phone/GetPlayerHouses', JSON.stringify({}), function(Houses){
                        SetupPlayerHouses(Houses);
                    });
                    $.post('https://lrp-phone/GetPlayerKeys', JSON.stringify({}), function(Keys){
                    ...
                    $.post('https://lrp-phone/GetCurrentLawyers', JSON.stringify({}), function(data){
                        SetupLawyers(data);
                    });
                } else if (PressedApplication == "store") {
                    $.post('https://lrp-phone/SetupStoreApps', JSON.stringify({}), function(data){
                        SetupAppstore(data);
                    });
                } else if (PressedApplication == "trucker") {
                    $.post('https://lrp-phone/GetTruckerData', JSON.stringify({}), function(data){
                        SetupTruckerInfo(data);
                    });
                }
                else if (PressedApplication == "gallery") {
                    $.post('https://lrp-phone/GetGalleryData', JSON.stringify({}), function(data){
...
$(document).on('click', '.mykeys-key', function(e){
    ...
    $.post('https://lrp-phone/SetHouseLocation', JSON.stringify({
        HouseData: KeyData
    }))
});
...
QB.Phone.Notifications.Add = function(icon, title, text, color, timeout) {
    $.post('https://lrp-phone/HasPhone', JSON.stringify({}), function(HasPhone){
        if (HasPhone) {
            ...
            if (QB.Phone.Notifications.Timeout == undefined || QB.Phone.Notifications.Timeout == null) {
                ...
                if (!QB.Phone.Data.IsOpen) {
                    QB.Phone.Animations.BottomSlideUp('.container', 300, -60);
                }
...
QB.Phone.Functions.UpdateTime = function(data) {
    ...
    $("#phone-server").html("<span>" + data.InGameTime.hour + ":" + data.InGameTime.minute + "</span>");
}
...
QB.Screen.popUp = function(source){
    if(!up){
        ...
        $('<img  src='+source+' style = "width:20%; left: 60%;bottom: 35%;position: absolute;">').appendTo('.popupclass')
        up = true
    }
}
