...
function LoadDebtJob(){
    ...
    $.post('https://lrp-phone/GetHasBills_debt', JSON.stringify({}), function(HasTable){
...
$(document).on('click', '#debt-create-bill-ended', function(e){
    ...
    if ((ID && Amount && Reason) != "" && (ID && Amount) >= 1){
        $.post('https://lrp-phone/SendBillForPlayer_debt', JSON.stringify({
            ID: ID,
            Amount: Amount,
            Reason: Reason,
        }));
...
$(document).on('click', '#debt-create-bill-accept', function(e){
    ...
    if(PlyMoney >= ENDamount){
        $.post('https://lrp-phone/debit_AcceptBillForPay', JSON.stringify({
            id: ENDKey,
            Amount: ENDamount,
            CSN: ENDsenderCSN,
        }));
