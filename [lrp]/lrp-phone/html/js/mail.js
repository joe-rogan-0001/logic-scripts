...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://lrp-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://lrp-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://lrp-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://lrp-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
        }else {
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://lrp-phone/DeleteAdvert', function(){
        setTimeout(function(){
