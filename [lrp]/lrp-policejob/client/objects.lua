...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        --if PlayerJob.name == "police" and PlayerJob.onduty then
            ...
            SetEntityHeading(netid, GetEntityHeading(PlayerPedId()))
            PlaceObjectOnGroundProperly(netid)
            PlayEntityAnim(netid, "P_Stinger_S_Deploy", LoadDict("p_ld_stinger_s"), 1000.0, false, true, 0, 0.0, 0)
            while not IsEntityPlayingAnim(netid, "p_ld_stinger_s", "P_Stinger_S_Deploy", 3) do
               Wait(0)
           end
           SetEntityVisible(netid, true)
           while IsEntityPlayingAnim(netid, "p_ld_stinger_s", "P_Stinger_S_Deploy", 3) and GetEntityAnimCurrentTime(netid, "p_ld_stinger_s", "P_Stinger_S_Deploy") <= 0.99 do
               Wait(0)
           end
           PlayEntityAnim(netid, "p_stinger_s_idle_deployed", LoadDict("p_ld_stinger_s"), 1000.0, false, true, 0, 0.99, 0)
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        --if PlayerJob.name == "police" and PlayerJob.onduty then
                            DrawText3D(pos.x, pos.y, pos.z, '[E] Pickup Spikes')
