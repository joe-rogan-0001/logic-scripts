...
local function GetCuffedAnimation(cuffer)
	loadAnimDict("mp_arrest_paired")
...
Citizen.CreateThread( function()
    ...
    exports["lrp-keymapping"]:registerKeyMapping("", "Player", "Carry person", "+Carry", "-Carry")

	RegisterCommand("+policeCuff", function()
        TriggerEvent('police:server:CuffPlayer')
    end, false)
	RegisterCommand('-policeCuff', function() end, false)
	exports["lrp-keymapping"]:registerKeyMapping("", "Player", "Cuff / Uncuff", "+policeCuff", "-policeCuff")
	...
	exports["lrp-keymapping"]:registerKeyMapping("", "Player", "Escort", "+policeEscort", "-policeEscort")
	...
	exports["lrp-keymapping"]:registerKeyMapping("", "Player", "Force into Vehicle", "+policeSeat", "-policeSeat")
	...
	exports["lrp-keymapping"]:registerKeyMapping("", "Player", "Soft Cuff", "+policeSoft", "-policeSoft")
...
RegisterNetEvent('police:client:CuffPlayerHardPD', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QBCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
...
RegisterNetEvent('police:client:fcuff')
AddEventHandler('police:client:fcuff', function()
    isHandcuffed = false
    isEscorted = false
    TriggerEvent('hospital:client:isEscorted', isEscorted)
    DetachEntity(PlayerPedId(), true, false)
