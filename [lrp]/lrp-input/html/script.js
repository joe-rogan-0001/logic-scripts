...
const OpenMenu = (data) => {
    ...
    $("#lrp-input-form").on("change", function (event) {
        formInputs[$(event.target).attr("name")] = $(event.target).val();
    });

    $("#lrp-input-form").on("submit", async function (event) {
        if (event != null) {
            event.preventDefault();
        }
        let formData = $("#lrp-input-form").serializeArray();
