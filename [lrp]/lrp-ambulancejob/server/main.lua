...
RegisterNetEvent('hospital:server:SendToSurgeryBed', function(bedId, isRevive)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	TriggerClientEvent('hospital:client:SendToBed', src, bedId, Config.Locations["beds2"][bedId], isRevive)
	TriggerClientEvent('hospital:client:SetSurgeryBed', -1, bedId, true)
end)

RegisterNetEvent('hospital:server:billplayer', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('hospital:server:RespawnAtHospital', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	for k, v in pairs(Config.Locations["respawnbeds"]) do
		if not v.taken then
	...
	TriggerClientEvent('hospital:client:SendToBed', src, 1, Config.Locations["respawnbeds"][1], true)
...
RegisterNetEvent('qb-ambulancejob:server:putonbed', function(playerId)
    local src = source
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
            TriggerClientEvent("qb-ambulancejob:main:putonbed", EscortPlayer.PlayerData.source)
    end
end)

RegisterNetEvent('qb-ambulancejob:server:putonsurgerybed', function(playerId)
    local src = source
    local EscortPlayer = QBCore.Functions.GetPlayer(playerId)
    if EscortPlayer then
            TriggerClientEvent("qb-ambulancejob:main:putonsurgerybed", EscortPlayer.PlayerData.source)
    end
end)