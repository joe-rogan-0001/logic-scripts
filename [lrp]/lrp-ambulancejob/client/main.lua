...
local function SetClosestBed2()
    local pos = GetEntityCoords(PlayerPedId(), true)
    local current2 = nil
    local dist = nil
    for k, v in pairs(Config.Locations["beds2"]) do
        local dist3 = #(pos - vector3(Config.Locations["beds2"][k].coords.x, Config.Locations["beds2"][k].coords.y, Config.Locations["beds2"][k].coords.z))
        if current2 then
            if dist3 < dist then
                current2 = k
                dist = dist3
    ...
    if current2 ~= closestBed2 and not isInHospitalBed then
        closestBed2 = current2
    end
end
...
local function SetBedCam()
    ...
    SetEntityCoords(player, bedOccupyingData.coords.x, bedOccupyingData.coords.y, bedOccupyingData.coords.z + 0.00)
...
RegisterNetEvent('hospital:client:SetSurgeryBed', function(id, isTaken)
    Config.Locations["beds2"][id].taken = isTaken
end)


RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    TriggerServerEvent("hospital:server:RespawnAtHospital")
    if exports["lrp-policejob"]:IsHandcuffed() then
...
RegisterNetEvent('qb-ambulancejob:job:statuscheck')
AddEventHandler('qb-ambulancejob:job:statuscheck', function()
    CheckWeaponDamage(PlayerPedId())
end)

RegisterNetEvent('qb-ambulancejob:job:checkin')
AddEventHandler('qb-ambulancejob:job:checkin', function()
           -- TriggerEvent('animations:client:EmoteCommandStart', {"notepad"})
            QBCore.Functions.Progressbar("hospital_checkin", "Checking in..", 2000, true, true, {
                ...
                if bedId then
                    ...
                    TriggerServerEvent("hospital:server:billplayer")
                else
...
RegisterNetEvent('qb-ambulancejob:main:lieonsurgerybed')
AddEventHandler('qb-ambulancejob:main:lieonsurgerybed', function()
    local pos = GetEntityCoords(PlayerPedId())
    if closestBed2 and not isInHospitalBed then
        if #(pos - vector3(Config.Locations["beds2"][closestBed2].coords.x, Config.Locations["beds2"][closestBed2].coords.y, Config.Locations["beds2"][closestBed2].coords.z)) < 1.7 then
        sleep = 7
            if GetAvailableBed(closestBed2) then
                TriggerServerEvent("hospital:server:SendToSurgeryBed", closestBed2, false)
            else
                QBCore.Functions.Notify("Beds are occupied..", "error")
            end
        end
...
RegisterNetEvent('qb-ambulancejob:main:lieonbed')
AddEventHandler('qb-ambulancejob:main:lieonbed', function()
    local pos = GetEntityCoords(PlayerPedId())
    if closestBed and not isInHospitalBed then
        if #(pos - vector3(Config.Locations["beds"][closestBed].coords.x, Config.Locations["beds"][closestBed].coords.y, Config.Locations["beds"][closestBed].coords.z)) < 1.7 then
        sleep = 7
...
RegisterNetEvent('qb-ambulancejob:main:putplayeronbed', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 1.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("qb-ambulancejob:server:putonbed", playerId)
        end
    else
...
RegisterNetEvent('qb-ambulancejob:main:putplayeronsurgerybed', function()
    local player, distance = QBCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 1.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("qb-ambulancejob:server:putonsurgerybed", playerId)
        end
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
...
RegisterNetEvent('qb-ambulancejob:main:putonbed', function()
    local ped = PlayerPedId()
    if isEscorted then
        if GetAvailableBed(closestBed) then
            TriggerServerEvent("hospital:server:SendToBed", closestBed, false)           
...
RegisterNetEvent('qb-ambulancejob:main:putonsurgerybed', function()
    local ped = PlayerPedId()
    if isEscorted then
        if GetAvailableBed(closestBed2) then
            TriggerServerEvent("hospital:server:SendToSurgeryBed", closestBed2, false)           
...
RegisterNetEvent('qb-ambulancejob:main:getoutofbed')
AddEventHandler('qb-ambulancejob:main:getoutofbed', function()
    while true do
        sleep = 1000
        if isInHospitalBed and canLeaveBed then
            sleep = 0
...
exports['lrp-target']:AddBoxZone("pillbox6", vector3(-255.21, 6332.38, 32.43), 2.0, 5.0, {
    name = "pillbox5",
    heading = 130.97,
    debugPoly = false,
    minZ = 26.28,
...
exports['lrp-target']:AddBoxZone("pillboxsurgery",vector3(321.15, -568.23, 44.26), 2.0, 2.0, {
    name = "pillboxsurgery",
    heading = 250,
    debugPoly = false,
    minZ = 41.28,
...
exports['lrp-target']:AddBoxZone("pillboxsurgery2", vector3(326.82, -570.96, 44.23), 2.0, 2.0, {
...
exports['lrp-target']:AddBoxZone("pillboxsurgery3", vector3(315.34, -566.38, 44.26), 2.0, 2.0, {
