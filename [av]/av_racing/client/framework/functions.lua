customModel = "bzzz_prop_light_tube_"
...
function AddCheckpoint(checkpointId)
    ...
    if checkpointId then
        ...
        if PileLeft then
            ...
            DeleteClosestObject(CreatorData.Checkpoints[tonumber(checkpointId)].offset.left, myPileModel)
            PileLeft = nil
        end
        ...
        if PileRight then
            ...
            DeleteClosestObject(CreatorData.Checkpoints[tonumber(checkpointId)].offset.right, myPileModel)
            PileRight = nil
        end
...
function SetupPiles()
    for k, v in pairs(CreatorData.Checkpoints) do
        if not CreatorData.Checkpoints[k].pileleft then
            CreatorData.Checkpoints[k].pileleft = CreatePile(v.offset.left, myPileModel)
        end

        if not CreatorData.Checkpoints[k].pileright then
            CreatorData.Checkpoints[k].pileright = CreatePile(v.offset.right, myPileModel)
        end
    end
end
...
function CreatePile(offset, model)
    ...
    local retval, groundZ, normal = GetGroundZAndNormalFor_3dCoord(offset.x, offset.y, offset.z)
--    print(offset.z, normal.z, (offset.z - normal.z))
    SetEntityCoords(Obj, offset.x, offset.y, offset.z - (normal.z / 2))
    PlaceObjectOnGroundProperly(Obj)
    PlaceObjectOnGroundProperly_2(Obj)
    return Obj
end
...
function handleFlare (checkpoint)
    if CurrentRaceData.Checkpoints and CurrentRaceData.Checkpoints[checkpoint] then 
        local Size = 1.0
        local left = showNonLoopParticle('core', 'exp_grd_flare', CurrentRaceData.Checkpoints[checkpoint].offset.left, Size)
        local right = showNonLoopParticle('core', 'exp_grd_flare', CurrentRaceData.Checkpoints[checkpoint].offset.right, Size)
