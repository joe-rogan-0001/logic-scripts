AddEventHandler("av_music:openMenu", function()
    local job = GetJob()
    local jobName = job.name
    if Config.RecordLabels[jobName] then
        local options = {
            {
                title = Lang['add_song'],
        ...
        lib.registerContext({
            id = 'main_music_menu',
            title = Config.RecordLabels[jobName].label,
            onExit = function()
...
AddEventHandler("av_music:addSong", function(data)
    ...
    if Config.RecordLabels[jobName] then
        local input = lib.inputDialog(Config.RecordLabels[jobName].label, {Lang['artist_name'], Lang['song_name'], Lang['song_url']})
    ...
    if songs and songs[1] then
        ...
        for k,v in pairs(songs) do
            options[#options+1] = {
                ...
                args = {title = Config.RecordLabels[jobName].label, data = v}
            }
            lib.registerContext({
                id = 'burn_cd_menu',
                title = Config.RecordLabels[jobName].label,
        ...
        TriggerEvent("av_laptop:notification", Config.RecordLabels[jobName].label, Lang["no_songs"], 'error')
    end
end)
...
AddEventHandler("av_music:setCount", function(args)
    local input = lib.inputDialog(args.title, {Lang['cd_count']..' '..Lang['cd_max']..'('..Config.MaxCDs..')'})
    if not input then return end
    if not tonumber(input[1]) then return end
    if tonumber(input[1]) > Config.MaxCDs then return end
