...
Citizen.CreateThread(function()
    ...
    while true do
        Citizen.Wait(0)
        if IsControlPressed(0, 303) or IsDisabledControlPressed(0, 303) then
...
function DrawText3DTalking(x,y,z, text, textColor)
    ...
    local serverid = tonumber(GetPlayerServerId(PlayerId()))
    serverid = (" " .. serverid .. " " )
    if onScreen then
        if text == serverid then
            SetTextScale(0.0*scale, 0.75*scale)
            SetTextFont(4)
            SetTextProportional(1)
            SetTextColour(127,178,125, 255)
